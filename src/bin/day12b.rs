...
impl Caves {
    ...
    pub fn path2names<'a>(&self, iter: impl Iterator<Item = &'a Cave>) -> Vec<String> {
        iter.map(|c| self.names[c.0].clone()).collect()
    }
...
fn dfs(
    ...
    for cave in caves.iter_next(current) {
        if cave == start || (cave != end && visits[cave.0] > 0 && twice) {
            continue;
        }
        path.push(*cave);
        if !caves.is_big(cave) {
            visits[cave.0] += 1;
        }
        numpaths += dfs(
            caves,
...
fn process(bufin: impl BufRead) -> Result<usize> {
    ...
    let mut visits = [0].repeat(caves.names.len());
    visits[start.0] = 2;
    visits[end.0] = 2;
    let paths = dfs(&caves, &start, &start, &end, &mut path, &mut visits, false);
    Ok(paths)
}
...
#[test]
fn test1() -> Result<()> {
    do_test(&adventofcode2021::examples::DAY12_1, 36)
}

#[test]
fn test2() -> Result<()> {
    do_test(&adventofcode2021::examples::DAY12_2, 103)
}

#[test]
fn test3() -> Result<()> {
    do_test(&adventofcode2021::examples::DAY12_3, 3509)
}

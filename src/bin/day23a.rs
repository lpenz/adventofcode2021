...
type Sqrid = sqrid::sqrid_create!(13, 3, false);
...
pub const QAS: [Qa; 2] = [QAA2, QAA1];
pub const QBS: [Qa; 2] = [QAB2, QAB1];
pub const QCS: [Qa; 2] = [QAC2, QAC1];
pub const QDS: [Qa; 2] = [QAD2, QAD1];
pub const AMPHICOLUMN: [(Amphi, [Qa; 2]); 4] = [
    (Amphi::A, QAS),
    (Amphi::B, QBS),
    (Amphi::C, QCS),
    (Amphi::D, QDS),
...
#[test]
fn test_hardcoded() -> Result<()> {
    let input = adventofcode2021::examples::DAY23;
    let mut node = parser::parse(input.as_bytes())?;
    let acts = vec![
        Action::new(QAC1, Qa::new::<4, 0>()),
        Action::new(QAB1, Qa::new::<7, 1>()),
        Action::new(QAB2, Qa::new::<6, 0>()),
        Action::new(Qa::new::<4, 0>(), QAB2),
        Action::new(QAA1, QAB1),
        Action::new(QAD1, Qa::new::<8, 0>()),
        Action::new(QAD2, Qa::new::<10, 0>()),
        Action::new(Qa::new::<8, 0>(), QAD2),
        Action::new(Qa::new::<6, 0>(), QAD1),
        Action::new(Qa::new::<10, 0>(), QAA1),
    ];
    for a in acts {
        node.apply(&a);
    }
    assert_eq!(node.cost, 12521);
    Ok(())
}
